...
echo ""
echo -e "${YELLOW}=== Alembic Migration Notes ===${NC}"
echo "  Migrations run automatically on container startup via entrypoint.sh"
echo "  To create a new migration locally:"
echo "    cd backend && source .venv/bin/activate"
echo "    alembic revision --autogenerate -m 'description'"
echo "    alembic upgrade head  # test locally first"
echo ""
echo -e "${YELLOW}⚠️  FIRST DEPLOY with Alembic on existing DB:${NC}"
echo "  If the DB already has tables (created by create_all), stamp the baseline:"
echo "    alembic stamp 377130333335"
echo "  This tells Alembic the DB is already at the baseline version."
