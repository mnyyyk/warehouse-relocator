...
const OptimizePage: NextPage & { pageTitle?: string } = () => {
  ...
  const [touchBudgetInput, setTouchBudgetInput] = useState<string>('100');
  ...
  const [fillRate, setFillRate] = useState<number>(0.90);
        ...
        es.onmessage = (ev) => {
          ...
          try {
            ...
            if (evt.type === 'planned' && typeof evt.count === 'number') {
              ...
              // 総合評価レポートを受信
              console.log('[SSE] Summary report event received:', evt);
              if (evt.report && typeof evt.report === 'string') {
                setSummaryReport(evt.report);
                console.log('[SSE] Summary report SET:', evt.report.substring(0, 100) + '...');
              } else {
                console.warn('[SSE] Summary report event but no valid report field:', evt);
              }
              if (evt.message) {
                setReloStatus(evt.message);
              }
            } else if (evt.type === 'done') {
              if (typeof evt.accepted === 'number') setLiveAccepted(evt.accepted);
              setReloStatus(`最適化が完了しました（accepted=${evt.accepted ?? '-'}）`);
              // SSE接続は少し遅延させて閉じる（summary_reportを受信するため）
              setTimeout(() => {
                try { es.close(); } catch {}
                esRef.current = null;
                setUsingSSE(false);
  ...
  // keep trace id in meta; live polling uses /relocation/debug snapshot
      ...
      // 最適化完了後にサマリーレポートを取得（trace_idを指定）
      try {
        const traceParam = reloMeta?.trace_id ? `?trace_id=${encodeURIComponent(reloMeta.trace_id)}` : '';
        const { json: debugJson } = await getWithFallback(`/v1/upload/relocation/debug${traceParam}`);
        if (debugJson?.summary_report && typeof debugJson.summary_report === 'string') {
          setSummaryReport(debugJson.summary_report);
          console.log('[Relocation complete] Summary report retrieved:', debugJson.summary_report.substring(0, 100) + '...');
        } else {
          console.warn('[Relocation complete] No summary_report in debug response');
        }
      } catch (err) {
        console.warn('[Relocation complete] Failed to retrieve summary report:', err);
      }
    } catch (e: any) {
    ...
    const tick = async () => {
      try {
        ...
        if (json?.summary_report && typeof json.summary_report === 'string') {
          ...
          console.log('[Polling] Summary report retrieved:', json.summary_report.substring(0, 100) + '...');
        }
      } catch {
  ...
  const SummaryPanel = ({ data }: { data: any }) => {
    if (!data) return null;
    const num = (v: any, d = 0) => (typeof v === 'number' ? v.toFixed(d) : String(v ?? ''));
    return (
      <div style={{ border: '1px solid #ddd', borderRadius: 8, padding: 12, margin: '12px 0', background: '#fafafa' }}>
        <div style={{ fontWeight: 600, marginBottom: 8 }}>効率化サマリー</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8 }}>
          <div>移動件数: {data.moves}</div>
          <div>対象SKU数: {data.unique_skus}</div>
          <div>触ったロケ数: {data.locations_touched}</div>
          <div>ケース合計: {data.qty_cases_total}</div>
          <div>平均段改善: {num(data.avg_level_improvement, 2)}</div>
          <div>平均列距離: {num(data.avg_column_distance, 2)}</div>
          <div>平均奥行距離: {num(data.avg_depth_distance, 2)}</div>
          <div>取口(1-2段)への移動: {data.moves_to_pick_levels}</div>
          <div>取りやすさ改善合計: {num(data.ease_gain_sum)}</div>
          <div>取りやすさ改善平均: {num(data.ease_gain_avg, 2)}</div>
        </div>
        {Array.isArray(data.highlights) && data.highlights.length > 0 && (
          <div style={{ marginTop: 8 }}>
            <div style={{ fontWeight: 600, marginBottom: 4 }}>ハイライト（抜粋）</div>
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              {data.highlights.map((h: any, i: number) => (
                <li key={i}>
                  SKU {h.sku_id} Lot {h.lot || '-'}: {h.from_loc} → {h.to_loc}（段Δ {h.level_change}, 列Δ {h.column_change}, 奥行Δ {h.depth_change}, 取りやすさ +{h.ease_gain}）
                </li>
              ))}
            </ul>
          </div>
  ...
  const exportFinalMovesOnly = useCallback(async () => {
    ...
    try {
      ...
      // CSV形式でエクスポート
      const header = ['sku_id', 'lot', 'lot_date', 'qty', 'from_loc', 'to_loc', 'distance', 'is_consolidated', 'original_steps', 'intermediate_locations'];
      const lines = [
        header.join(','),
  ...
  return (
    <main role="main" className="space-y-8">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div style={{ background: '#fff3cd', padding: '8px', fontSize: '12px', fontFamily: 'monospace' }}>
          summaryReport state: {summaryReport ? `EXISTS (${summaryReport.length} chars)` : 'NULL'}
        </div>
      )}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <label className="text-sm">
            ...
            <input
              ...
              max={1.0}
              value={fillRate}
              onChange={(e) => setFillRate(parseFloat(e.target.value || '0.90'))}
