...
async function getWithFallback(path: string): Promise<{ json: any; via: 'proxy' | 'direct' }> {
  try {
    ...
    if (!res.ok) {
      const errorMsg = json?.detail || json?.message || res.statusText;
    ...
    try {
      ...
      if (!res2.ok) {
        const errorMsg = json2?.detail || json2?.message || res2.statusText;
        throw new Error(errorMsg);
      }
...
const DebugPage: NextPage & { pageTitle?: string } = () => {
  ...
  // --- Render (padding/widthはレイアウト側に任せる) ---
  return (
