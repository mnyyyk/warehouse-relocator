...
// ===== component =====
const AnalyzePage: NextPage & { pageTitle?: string } = () => {
  // 共通パラメータ
  ...
  // 初回自動実行＆期間/品質変更時に自動で再計算→ダッシュボード更新（本体は refreshDashboard 定義後に配置）
  ...
  const refreshDashboard = useCallback(async () => {
    ...
    try {
      ...
      // できるだけ多めに取得してクライアント側で降順ソート
      const acc: SkuMetric[] = [];
      const limit = 500;
      let offset = 0;
      let total = 0;
      do {
        // ★ window_days を必須クエリとして付与（バックエンドは必須）
        const params = new URLSearchParams();
        // 無期限(0)はサーバ側では 99999 を使用しているため整合させる
        const wd = windowDays === 0 ? 99999 : windowDays;
        params.set('window_days', String(wd));
        params.set('limit', String(limit));
        params.set('offset', String(offset));
        // 品質でサーバー側も絞りたい場合は繰り返し指定（任意）
      ...
      // もしデータが少ない場合やフィールドが不完全な場合はダミーで補完
      ...
      do {
        ...
        rows.forEach((r: any) => {
          ...
          if (qualities.length && !qualities.includes(q)) return; // 品質フィルタが選択されていればそれに限定
          qCount.set(q, (qCount.get(q) ?? 0) + 1);
        });
        invOffset += rows.length;
      } while (invOffset < invTotal && invOffset < 10000); // safety cap
      ...
      // 3) ロケーション概要（ページネーション対応 / booleanフラグ or 集計行の両対応）
      const isTrue = (v: any) => {
        if (v === true) return true;
        if (typeof v === 'number') return v === 1; // 0/1 のときのみ true
        if (typeof v === 'string') {
      ...
      let locOffset = 0;
      let locTotalHeader = 0; // APIが返す total（信用できないことがあるので参考値）
      let locationTotalAcc = 0; // 表示用の総ロケ数
      let canReceiveAcc = 0;
      let highnessAcc = 0;
      ...
      do {
        ...
        for (const r of rows) {
          // 品質フィルタ（集計行でも quality_name があれば効かせる）
          ...
          // 集計行の可能性：total_slots がある、または can_receive が 0/1 を超える値
          ...
          if (isAggregated) {
            // 例）{ total_slots: 5000, can_receive: 2765, cannot_receive: 2235, highness: 198 }
            ...
            if (totalSlots > 0) {
              ...
              // 集計行だが総数が読み取れない場合は行数で+1
              locationTotalAcc += 1;
            }
            canReceiveAcc += canRecv;
            highnessAcc += high;
          } else {
            // per-location 行（boolean系）
            locationTotalAcc += 1;
  ...
  // 自動実行: 初回マウント時と windowDays / qualities が変わったときに
  // 1) 分析を実行 → 2) ダッシュボード更新 を順に行う
  useEffect(() => {
    (async () => {
      try {
        await startAnalyze();
...
AnalyzePage.pageTitle = '分析（前処理 & ダッシュボード）';
